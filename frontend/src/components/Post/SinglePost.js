...
import { HeartRegular, HeartFilled, ShareRegular, BookmarkRegular, BookmarkFilled, DismissRegular } from '@fluentui/react-icons';
...
const SinglePost = () => {
  ...
  const visibleComments = post && post.comments 
  ? showAllComments 
    ? post.comments 
  ...
  const getMediaUrl = (mediaPath) => {
    if (!mediaPath || mediaPath === 'undefined') {
      return `https://ui-avatars.com/api/?name=User`; // Default avatar
    }
  ...
  const handleCommentReply = async (commentId, replyText) => {
    try {
      ...
      // After adding reply, fetch the full post data to ensure we have all media URLs
  ...
  const handleComment = async (e) => {
    ...
    try {
      ...
      const updatedPost = await response.json();
      // Verify we have all the data before updating state
      if (updatedPost && updatedPost.media === undefined) {
        // If media is missing, fetch the full post again
...
{/* Comments section with show more functionality */}
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start flex-1">
            <img
              ...
              onError={(e) => {
                e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(comment.user.username)}`;
              }}
            />
            <div className="ml-3 flex-1">
              ...
              {/* Only show reply input and replies if showAllComments is true */}
              {showAllComments && (
                <>
...
{comment.replies?.length > 0 && showAllComments && (
        ...
        <div className="flex items-start flex-1">
          <img
            src={reply.user?.profilePicture 
              ...
              : `https://ui-avatars.com/api/?name=${encodeURIComponent(reply.user?.username || 'User')}`
            }
